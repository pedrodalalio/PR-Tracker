...
const createMockWorkoutExercise = (
  exercise: Exercise,
  index: number,
): WorkoutExercise => ({
...
const createProgressiveMockWorkoutExercise = (
  exercise: Exercise,
  index: number,
  weekOffset: number,
): WorkoutExercise => {
  const baseWeight =
    exercise.category === "Cardio" ? 0 : Math.floor(Math.random() * 30) + 30;
  const progressionMultiplier = Math.max(0.8, 1 - weekOffset * 0.02); // Progressive overload over time

  const sets: Set[] = [];
  const setCount =
    exercise.category === "Cardio" ? 1 : Math.floor(Math.random() * 2) + 3; // 3-4 sets for strength, 1 for cardio
  ...
  for (let i = 0; i < setCount; i++) {
    if (exercise.category === "Cardio") {
      sets.push({
        ...
        weight: 0,
        duration: Math.floor(
          (Math.random() * 900 + 1200) * progressionMultiplier,
        ), // 20-35 minutes
        distance: Math.floor(
          (Math.random() * 4000 + 3000) * progressionMultiplier,
        ), // 3-7km
      });
    } else {
      sets.push({
        ...
        weight: Math.floor(
          baseWeight * progressionMultiplier + (Math.random() * 10 - 5),
        ), // Progressive weight with variation
      });
    }
  }
...
const generateMockWorkouts = (): Workout[] => {
  ...
  for (let week = 0; week < 12; week++) {
    ...
    for (
      let workoutIndex = 0;
      workoutIndex < workoutsThisWeek;
      workoutIndex++
    ) {
      const daysAgo =
        week * 7 + workoutIndex * 2 + Math.floor(Math.random() * 2);
      const workoutType =
        workoutTypes[Math.floor(Math.random() * workoutTypes.length)];
      const workoutNameOptions =
        workoutNames[workoutType as keyof typeof workoutNames];
      const workoutName =
        workoutNameOptions[
          Math.floor(Math.random() * workoutNameOptions.length)
        ];
      ...
      const endTime = new Date(
        workoutDate.getTime() -
          (60 + Math.floor(Math.random() * 60)) * 60 * 1000,
      );
      ...
      switch (workoutType) {
        ...
        case "cardio":
          ...
          break;
        case "full":
          const upperEx = mockExercises
            .filter((e) => e.category === "Upper")
            .slice(0, 2);
          const lowerEx = mockExercises
            .filter((e) => e.category === "Lower")
            .slice(0, 2);
          const cardioEx = mockExercises
            .filter((e) => e.category === "Cardio")
            .slice(0, 1);
...
export class MockDataService {
  ...
  async createWorkout(request: CreateWorkoutRequest): Promise<Workout> {
    ...
    const newWorkout: Workout = {
      ...
      exercises:
        request.exercises?.map((ex, index) => ({
          ...ex,
          id: `we-${Date.now()}-${index}`,
        })) || [],
    };
