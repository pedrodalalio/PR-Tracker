...
export async function goalsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{}>('/goals', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  // Update user goals
  fastify.put<{ Body: UpdateGoalsRequest }>('/goals', async (request: FastifyRequest<{ Body: UpdateGoalsRequest }>, reply: FastifyReply) => {
    try {
  ...
  fastify.get<{}>('/goals/week-progress', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.get<{}>('/goals/streak-info', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const goals = await prisma.userGoals.findFirst();
      if (!goals) {
        return reply.status(404).send({ error: 'Goals not found' });
  ...
  fastify.post<{}>('/goals/update-streak', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const goals = await prisma.userGoals.findFirst();
      if (!goals) {
        return reply.status(404).send({ error: 'Goals not found' });
      ...
      if (workoutDates.length > 0) {
        ...
        const lastWorkoutDateOnly = new Date(workoutDates[0] + 'T00:00:00.000Z');
        ...
        if (lastWorkoutDateOnly >= yesterday) {
          ...
          while (workoutIndex < workoutDates.length) {
            const workoutDate = new Date(workoutDates[workoutIndex] + 'T00:00:00.000Z');
